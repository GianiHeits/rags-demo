...
class RAG:
    def __init__(self, streaming=False):
        ...
        self.blip_model = BlipForConditionalGeneration.from_pretrained(
            self.blip_model_type
        )

        self.blip_qna_model_type = "Salesforce/blip-vqa-base"
        self.blip_qna_processor = BlipProcessor.from_pretrained(
            self.blip_qna_model_type
        )
        self.blip_qna_model = BlipForQuestionAnswering.from_pretrained(
            self.blip_qna_model_type
        )
    ...
    def prepare_vectorstores_for_demo(self):
        self.vectorstore_links_sources = [
            "https://heits.digital/articles/gpt3-overview"
        ]
        self.vectorstore_links = self._get_vectorstore_from_links(
            self.vectorstore_links_sources
        )

        self.vectorstore_youtube_sources = (
            "https://www.youtube.com/watch?v=lW7Mxj8KUJE&ab_channel=LinusTechTips"
        )
        self.vectorstore_youtube = self._get_vectorstore_from_youtube(
            self.vectorstore_youtube_sources
        )

        self.vectorstore_images_sources = [
            str(path) for path in Path("data/images/").glob("*")
        ]
        raw_images = [
            Image.open(img_path).convert("RGB")
            for img_path in self.vectorstore_images_sources
        ]
        self.vectorstore_images = self._get_extensive_vectorstore_from_images(
            raw_images, self.vectorstore_images_sources
        )
    ...
    def query_images(self, image_path_list: List[str], query: str) -> str:
        if image_path_list != self.vectorstore_images_sources:
            raw_images = [
                Image.open(img_path).convert("RGB") for img_path in image_path_list
            ]
            self.vectorstore_images_sources = raw_images
            self.vectorstore_images = self._get_extensive_vectorstore_from_images(
                raw_images, image_path_list
            )
    ...
    def _get_vectorstore_from_images(
        self,
        raw_images_list: List[Image.Image],
        raw_images_paths: List[str] = None,
        query: str = "",
    ):
        # Use this to ask GPT about content in images
        descriptions = self._extract_generic_image_info(raw_images_list)
        descriptions_questions = self._extract_image_info_based_on_questions(
            raw_images_list, question=query
        )
        ...
        for i, [desc1, desc2] in enumerate(zip(descriptions, descriptions_questions)):
            ...
            description = (
                "{"
                + f"file_name: '{source}', description_generic: '{desc1}', description_specific: '{desc2}'"
                + "}"
            )
    ...
    def _get_extensive_vectorstore_from_images(
        self, raw_images_list: List[Image.Image], raw_images_paths: List[str] = None
    ):
    ...
    def _get_questions_from_prompt(self, query):
        ...
        prompt = ChatPromptTemplate.from_template(
            "Pretend you are Sherlock Holmes and you ask as many questions in order to solve a mistery. What questions would ask about a image with the following description: {query}. Your questions need to start with `what`, `how` or `why`. {format_instructions}",
    ...
    def _extract_image_info_based_on_questions(self, raw_images: List, question=""):
        ...
        for image in raw_images:
            ...
            description = self.blip_qna_processor.decode(
    ...
    def _extract_extensive_image_info(self, raw_images: List):
        ...
        for image, description in zip(raw_images, base_descriptions):
            ...
            for question in questions["questions"]:
                ...
                answer = self.blip_qna_processor.decode(
                    out[0], skip_special_tokens=True
                )
                print("Answer: ", answer)
